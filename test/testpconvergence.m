...
% with p-refinement of Spectral FEM basis functions
%
clc;
clear all;
close all
...
% Initialize:
% 
Etotp  = zeros(length(pSch),1);
Ekinp  = zeros(length(pSch),1);
Ecoulp = zeros(length(pSch),1);
Eenucp = zeros(length(pSch),1); 
Excp   = zeros(length(pSch),1);
totdof = zeros(length(pSch),1);

for run = 1:length(pSch)
    meshoptsSch = struct('element_order',pSch(run), 'numel',numelSch, 'domain',domain, ...
                         'verbose',toplot, 'element_type','spectral');
%              
    meshdatainputSch = struct('meshtype',meshtypeSch, 'param',aSch, ...
                              'meshopts',meshoptsSch);

    meshoptsPois = struct('element_order',pPois(run), 'numel',numelPois, 'domain',domain, ...
                          'verbose',toplot, 'element_type','spectral');
%
    ...
    [lam, u, scf_converged, Etotp(run), Ekinp(run), Ecoulp(run), Eenucp(run), Excp(run), history, exitflag] = radialdft(problemdata, ...
...
hold on;
% 
plot(totdof, Etotp, 'r', 'LineWidth', 1.25)
